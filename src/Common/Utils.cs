...
namespace ADSL.Common
{
    ...
    public class Utils
    {
        ...
        public static string GetSubString(string p_SrcString, int p_StartIndex, int p_Length, string p_TailString)
        {
            ...
            {   // 当是日文或韩文时，从指定索引位置截取指定长度的字符串，如果长度超过了指定长度，截到字符串末尾
        ...
        /// <returns></returns>
        public static string GetSubString(string strSource, int startIndex, int endIndex)
        {
            try
            {
                if (endIndex < 0)
                ...
                return strSource.Substring(startIndex, len);
            }
            catch
            {
                return string.Empty;
        ...
        public static int SafeInt32(object objNum)
        {
            ...
            if (TypeValidator.IsNumeric(strNum))
            {
                ...
                try
                {
                    ...
                    foreach (string strs in single)
                    {
                        if (strs != "")
                            ...
                            {
                                case "email":
                                    if (FormValidator.IsValidDoEmail(strs.ToString()) == false)
                                        throw new Exception();
                                    break;
                                ...
                                case "timesect":
                                    ...
                                    if (TypeValidator.IsTime(splitetime[1].ToString()) == false || TypeValidator.IsTime(splitetime[0].ToString()) == false)
                                        throw new Exception();
                                    break;
        ...
        public static bool IsNumericList(string numList)
        {
            ...
            {
                if (!TypeValidator.IsNumeric(num))
                    return false;
            }
            return true;
        }
        ...
        public static string FormatSafeSQL(string param)
        {
        ...
        {
            HttpContext.Current.Response.Write(string.Format("<script type='text/javascript'>{0}</script>", javascript));
        }
    }
}
